...
public class DBUtil {
    ...
    public static int insertAll(String tableName, List<Map<String, Object>> datas) throws SQLException {
        ...
        try {
            /**从数据库连接池中获取数据库连接**/
            connection = SimpleJDBCUtils.getConnection();
    ...
    public static List<Map<String, Object>> query(String tableName,
            Map<String, Object> whereMap,int beIndex,int num) throws Exception {
			String whereClause = "";
			Object[] whereArgs = null;
			if (whereMap != null && whereMap.size() > 0) {
			Iterator<String> iterator = whereMap.keySet().iterator();
			...
			String limit = beIndex+","+num;
			return query(tableName, false, null, whereClause, whereArgs, null, null, null, limit);
    }
    /**
     * 执行sql条件参数绑定形式的查询
     *
